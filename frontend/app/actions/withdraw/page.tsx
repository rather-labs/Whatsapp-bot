...
export default function Home() {
  ...
  useEffect(() => {    
    if (withdrawData.whatsappNumber.length > 0 && client) {
      const getNonce = async () => {
        ...
        setMessage({
          user: BigInt(withdrawData.whatsappNumber),
          assets: BigInt(withdrawData.amount),
          nonce: nonce as bigint,
        });
        setLabel(`Sign authorization to withdraw ${withdrawData.amount} USDC from vault to your wallet`);
      }
      getNonce();
    }
  }, [withdrawData, setMessage, vaultAddress, client, setLabel]);
  ...
  async function submitAuthorization() {
    const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/transfers/withdraw`, {
      whatsappNumber: withdrawData.whatsappNumber,
