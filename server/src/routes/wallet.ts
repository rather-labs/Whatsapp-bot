...
import { getContract, formatEther, formatUnits, getAddress } from 'viem';
import db from '../config/database';
import { publicClient, USDC_CONTRACT_ADDRESS, USDC_ABI } from '../config/blockchain';
...
router.get('/balance', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    db.get('SELECT * FROM users WHERE whatsapp_number = ?', [req.user?.whatsappNumber], async (err: any, user: any) => {
      if (err) {
...
// Send USDC payment
router.post('/pay', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    db.get('SELECT * FROM users WHERE whatsapp_number = ?', [req.user?.whatsappNumber], async (err: any, user: any) => {
      ...
      db.get('SELECT whatsapp_number FROM users WHERE whatsapp_number = ?', [recipient], (err: any, recipientUser: any) => {
        if (recipientUser) {
          db.run(
            'UPDATE users SET wallet_balance = wallet_balance + ? WHERE whatsapp_number = ?',
...
// Buy USDC
router.post('/buy', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
// Sell USDC
router.post('/sell', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { amount } = req.body;
